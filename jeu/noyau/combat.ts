...
import type { ToucheDef } from "./definitions";
import { boiteEntite, definitionDe, detruire, type Entite } from "./entites";
...
/*
 * Résolution des coups d'un tick : hitboxes des combattants et entités.
 *
...
const GEL_CONTRE = 24;
/** Gel maximal d'un coup d'entité (seule la cible est figée). */
const GEL_ENTITE_MAX = 6;
...
type Touche = {
  ...
  zone: Boite;
  sens: 1 | -1;
  /** Coup de l'attaquant ; null pour une entité. */
  coup: string | null;
  /** Clé « groupe × 16 + place » d'une hitbox, -1 pour une entité. */
  cle: number;
  entite: Entite | null;
};
...
export function resoudreTouches(monde: Monde): void {
    ...
    for (const hb of coupDe(a)?.hitboxes ?? []) {
      ...
      for (const b of monde.combattants) {
        ...
        if (chevauche(zone, boiteDe(b))) {
          touches.push({ attaquant: a, cible: b, hb, zone, sens: a.orientation, coup: a.coup, cle, entite: null });
        }
      }
    }
  }
  ...
  for (const e of monde.entites) {
    ...
    // Un piège ne blesse pas lui-même : il se déclenche (cf. entites.ts).
    if (!def.touche || def.declencheur) continue;
    const zone = boiteEntite(e, def);
    for (const b of monde.combattants) {
      if (b.id === e.proprio || b.ko || b.horsJeu || estInvulnerable(b) || e.touches.includes(b.id)) continue;
      if (!chevauche(zone, boiteDe(b))) continue;
      const sens: 1 | -1 = def.touche.radial ? (b.x >= e.x ? 1 : -1) : e.vx !== 0 ? (e.vx > 0 ? 1 : -1) : e.orientation;
      touches.push({ attaquant: monde.combattants[e.proprio], cible: b, hb: def.touche, zone, sens, coup: null, cle: -1, entite: e });
    }
  }
...
/** Milieu de l'intersection entre la zone qui frappe et la cible : point d'impact des effets. */
...
function appliquer(t: Touche, monde: Monde): void {
  const { attaquant: a, cible: b, hb, entite } = t;
  if (entite) {
    if (entite.detruite) return;
    entite.touches.push(b.id);
    entite.touchesFaites++;
  } else {
    a.touches.push(t.cle);
  }
  const point = impact(t.zone, boiteDe(b));
  const defEntite = entite ? definitionDe(monde, entite) : null;

  // Contre : la cible ne subit rien et riposte ; un attaquant au corps à
  // corps reste figé, un projectile est anéanti.
  ...
  if (coupCible?.contre && dans(b.frame, coupCible.contre.de, coupCible.contre.a)) {
    ...
    if (entite) detruire(monde, entite, false);
    else a.gel = Math.max(a.gel, GEL_CONTRE);
    emettre(monde, { type: "contre", source: b.id, cible: a.id, ...point, cle: b.coup ?? "" });
    return;
  }

  const coup = t.coup !== null ? a.perso.coups[t.coup] : undefined;
  ...
  if (degats > 0) consommerUniques(b);

  // Le surplus d'un coup fatal ne compte pas : seuls les PV réellement ôtés.
  degats = Math.min(degats, b.pv);
  b.pv -= degats;
  a.degatsInfliges += degats;
  // L'ultime (et ce qui en découle) ne recharge pas la jauge qui vient de le payer.
  const ultime = coup?.jauge || coup?.ultime || defEntite?.ultime;
  if (!ultime) a.jauge = Math.min(JAUGE_MAX, a.jauge + Math.trunc((degats * GAIN_INFLIGE) / 1000));
  b.jauge = Math.min(JAUGE_MAX, b.jauge + Math.trunc((degats * GAIN_SUBI) / 1000));

  // Égratignure (ni recul ni hitstun) : blesse sans interrompre.
  const egratignure = hb.recul === 0 && hb.hitstun === 0;
  if (!egratignure) {
    const gel = hb.gel ?? Math.min(12, 3 + Math.trunc(degats / 12));
    if (entite) b.gel = Math.max(b.gel, Math.min(gel, GEL_ENTITE_MAX));
    else {
  ...
  const cle = hb.effet ?? t.coup ?? entite?.def ?? "";
  const evenement = { source: a.id, cible: b.id, ...point, valeur: degats, cle };
  if (b.pv === 0) {
    b.ko = true;
    ejecter(b, hb, t.sens);
  ...
  if (entite && defEntite?.touchesMax && entite.touchesFaites >= defEntite.touchesMax) detruire(monde, entite);
}

/** Interrompt la cible et la projette selon la touche. */
function ejecter(b: Combattant, hb: ToucheDef, sens: 1 | -1): void {
